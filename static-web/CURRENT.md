...
The integrated Steward time workspace is implemented on `main`. Current implementation uses the existing Chat Plan / StudyTimer inputs and shared StudyTimerDock. The Dock supports bounded Recovery reality capture — immediate Pause, optional rest duration/method/note, explicit Resume, optional re-entry outcome — and that same private reality is available to Steward Today and the existing private Daily Packet. PR #959 adds the bounded Capacity Projection on the existing Chat Plan: current capacity → plain-language evidence basis → current load → action → recheck, joined with the same break/re-entry reality. PR #960 closes the exam-Work product translation: Home now shows decision-changing capacity as a compact Work Strategy tied to the resolved cross-subject allocation and schedule; Steward consumes the same canonical Chat Plan presentation on its timeline and keeps capacity subordinate to execution; Review attribution remains upstream in Personal/Chat. The Human pre-gate passed Authority/Semantic, Exam Orchestrator, Steward browser, Private Control, Final Cross-subject, Shared Timer and real macOS PingFang Home+Steward visual gates after catching/fixing three real defects (15px Home label, oversized Steward strategy rail, and canonical snake_case Chat Plan presentation not being consumed by Steward). Raw Health remains Health-owned and long-run calibration remains Personal-owned; KianOS owns only the current product projection and execution evidence. Readiness/recovery/debt scores remain invalid and there is no autonomous Website replanning. Real-Mac Current rollout/readback for the merged #960 release is still UNKNOWN while Remote is offline.
