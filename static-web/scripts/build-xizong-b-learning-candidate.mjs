...
const recallSpine = {
  ...
  D8:'分型/自然史 → DKA/HHS → 慢性并发症 → 诊断/功能评估 → 长期管理/药物选择',
  D9:'LES屏障/清除/黏膜 → GERD表现/并发 → 客观证据 → 抑酸/维持/手术 → 失弛缓对照',
  D10:'胃黏膜损伤/萎缩 → HP → PUD → UGIB先复苏 → 穿孔/梗阻/癌变警报 → 药物失败后胃切除/重建并发症',
  D11:'Tumor Gate/壁深 → 食管癌 → 胃癌身份/扩散 → 胃癌证据/分期/治疗 → lymphoma/GIST → 整合分流',
  D12:'TB共同病理 → 肠结核/腹膜结核 → IBD共同底座 → CD/UC鉴别 → 诱导/维持治疗 → IBS功能性分流',
  D13:'腹膜感染/液体源 → 损伤出血vs污染 → 器官特异损伤 → 稳定性决定检查/探查/源控制',
  D14:'梗阻分类/部位 → 闭袢/绞窄血运风险 → 阑尾炎标准链/例外 → 腹股沟空间 → 嵌顿/绞窄复位与手术',
  D15:'癌入口/证据 → 围术准备边界 → 左右结肠切除 → 直肠保肛/造口 → 癌前分子 → 齿状线坐标 → 肛周鉴别',
  ...
  D19:'胆道管道/结石 → 影像/减压/T管 → 胆囊良性炎症 → 胆囊癌 → 胆管石/AOSC → 胆管癌/出血 → 先天/黄疸定位 → 肝脓肿',
  D20:'胰酶提前激活 → 自我消化/坏死 → 局部集合/器官衰竭 → 证据 → 支持/介入/慢胰 → 胰腺癌 → 功能性胰岛肿瘤',
  D21:'TH合成/作用/HPT反馈 → 甲亢确认/定位 → 常规治疗/甲危/特殊边界 → 甲减 → 结节炎症 → 四癌 → 外科安全/颈部残余',
  D22:'球束网髓 + HPA → 原醛 → 嗜铬 → 库欣 → D6反馈定位整合',
...
const partialSystemReconstructions = [
  {id:'PSR-1_GI_INPUT',after_when_ready:['D1','D2','D3','D4'],target:'motility/control → secretion/digestion → selective absorption → portal/lymph entry'},
  {id:'PSR-2_METABOLIC_NETWORK',after_when_ready:['M1','M2','M3','M4','M5','M6','M7','M8','M9','M10'],incremental:true,target:'enzyme/cofactor → carbon/ATP/redox → storage/mobilization → lipid transport → nitrogen/one-carbon → nucleotide supply → bilirubin/hepatic chemical handling'},
  {id:'PSR-3_ENDOCRINE_CONTROL',after_when_ready:['D5','D6','D7','D8','D21','D22','D23'],target:'energy state → common feedback language → pancreatic control/diabetes → thyroid/adrenal/calcium-GH localization'},
  {id:'PSR-4_GI_ABDOMINAL_CLINICAL',after_when_ready:['D9','D10','D11','D12','D13','D14','D15'],target:'movement/outlet vs mucosal/inflammatory vs bleeding/perforation/obstruction vs abdominal-space emergency vs organ-tumor localization'},
  {id:'PSR-5_HEPATOBILIARY_PANCREAS',after_when_ready:['D16','D17','D18','D19','D20'],target:'hepatocyte injury/reconstruction → portal+synthetic failure → tumor/biliary obstruction/jaundice → pancreatic enzyme or tumor failure'},
  {id:'PSR-6_INFORMATION_TUMOR',after_when_ready:['G1','G2','G3','G4','G5'],external_required:['O9 pathology tumor-general model'],target:'DNA storage → replication/expression/regulation → mutation+repair → clonal-control failure → pathology tumor behavior → organ application'}
];
...
const output = {
  schema:'kianos.xizong.system_learning_support.v1',status:'L_CANDIDATE',construction_status:'PHASE3_SEMANTIC_CLOSED_PENDING_PROGRESSIVE_COMPRESSION_AUDIT',authority:'CURRENT_PHASE0_3_COMPILED_LEARNING_CONSTRUCTION',system_id:'digestive-metabolic-endocrine-tumor',canonical_id:'B',scope:'ALL_38_CANONICAL_BLOCKS_SYSTEM_BELOW_ONLY',study_policy:'content/xizong/knowledge/learner/study-policy.json',medical_core_owner:'content/xizong/knowledge/systems/b-digestive-metabolic-endocrine-tumor/',system_knowledge_owner:'content/xizong/knowledge/systems/b-digestive-metabolic-endocrine-tumor/system.json',
