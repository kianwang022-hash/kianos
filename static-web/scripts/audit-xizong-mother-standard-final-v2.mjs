...
check('future_source_readiness',()=>{
  ...
  return {biochemistry:'BLOCKED_FUTURE_SOURCE',humanities:'BLOCKED_FUTURE_SOURCE',exam_geometry:'BLOCKED_FUTURE_SOURCE',late_stage:'DELTA_SLOTS_READY'};
});
...
console.log(JSON.stringify({
  schema:'kianos.xizong.mother-standard-final-audit.v2',
  learner_u:false,
  results,
  summary:{pass:results.length-failed.length,fail:failed.length}
},null,2));
if(failed.length){
  console.error('FAIL Xizong mother-standard final audit v2');
  process.exitCode=1;
}else{
  console.log('PASS Xizong mother-standard final audit v2');
}
