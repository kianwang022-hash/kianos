...
assert(systemWuReturn.includes('currentXizongSystemWuEvidence(storage, value.system_id, questions)'), 'repair-plan-not-scoped-to-current-wu');
...
// System→Block repair uses canonical Memory Repair tasks, preserves provenance, and never manufactures mastery evidence.
assert(systemWuReturn.includes("inboxKey:'kianos-xizong-repair-inbox-v1:xizong:'"), 'system-repair-bypasses-inbox');
assert(systemWuReturn.includes('nextMemory=setRepairTasks(memory,[...kept,...tasks]);'), 'system-repair-bypasses-visible-memory-repair');
assert(repairBridge.includes('kianos-xizong-repair-inbox-v1:'), 'block-inbox-consumer-missing');
assert(repairBridge.includes('const next = setRepairTasks(memory, [...preserved, ...incoming]);'), 'block-inbox-bypasses-visible-memory-repair');
assert(!repairBridge.includes('appendMemoryEvidence') && !repairBridge.includes('evidence.push'), 'inbox-import-promotes-mastery');
assert(repairBridge.includes('sourceQuestionIds,'), 'inbox-loses-question-provenance');
const durableWriteIndex = repairBridge.indexOf('if (!writeJson(XIZONG_MEMORY_STORAGE_KEY, next))');
