...
// This follows the established Value projection, not raw JSON field completeness.
function expectedText(value) {
  ...
  if (Array.isArray(value.nodes) && Array.isArray(value.edges)) {
    return [heading, ...value.nodes.flatMap(node => [node.label,node.meaning,node.detail]), ...value.edges.map(edge => edge.relation)].filter(Boolean);
  }
...
export async function testPoliticsSubjectReadability({ browser, base, out, report }) {
  const directory = path.join(out,'subject-readability');
  ...
  const files = ['src/styles/politics-readable.css','src/components/PoliticsFrameValue.astro','src/components/PoliticsFrameMap.astro','scripts/test-politics-subject-readability.mjs'];
  result.source_sha256 = Object.fromEntries(files.map(file => [file,sha(file)]));
  const context = await browser.newContext({viewport:{width:1440,height:1000}});
  const page = await context.newPage();
  page.on('pageerror', error => result.errors.push(error.message));
  const noOverflow = async label => {
    const observation = await page.evaluate(() => ({ width:innerWidth, scroll:document.documentElement.scrollWidth }));
    assert.ok(observation.scroll <= observation.width+1, `${label}: horizontal page overflow ${JSON.stringify(observation)}`);
  };
  const settle = async () => {
  ...
  const sampled = new Set(); const representatives = [];
  try {
    const rows = listPoliticsChapterPathsCurrent();
    for (const row of rows) {
      const frame = loadPoliticsFrameProjection(row.subject,row.chapter);
      const href = `${base}/politics/${row.subject}/${row.chapter}/`;
      result.current = href;
      ...
      const chapter = {subject:row.subject, chapter:row.chapter, units:frame.units.length, referenceOnly:frame.referenceOnly.length, source:frame.source, checked:[]};
      ...
      for (const unit of frame.units) {
        ...
        const active = page.locator(`[data-frame-unit="${unit.unit_id}"]`);
        await active.waitFor({state:'visible'}); await settle();
        assert.equal(await page.locator('[data-frame-unit]:visible').count(),1);
        const primary = active.locator('.framePrimary');
        const actual = normalize(await primary.innerText());
        ...
        const maps = values.flatMap(mapsIn);
        const actualMaps = primary.locator('[data-frame-map]');
        assert.equal(await actualMaps.count(),maps.length,`${result.current}: map count`);
        for (let index=0;index<maps.length;index++) {
          const map = maps[index]; const rendered = actualMaps.nth(index);
          assert.equal(await rendered.locator('[data-graph-node]').count(),map.nodes.length);
          ...
          if (await rendered.locator('[data-acyclic="true"]').count()) {
            await rendered.locator('[data-graph-ready="true"]').waitFor({state:'attached'});
            assert.equal(await rendered.locator('[data-edge-path]').count(),map.edges.length);
          }
          result.maps++; result.edges+=map.edges.length;
        }
        const style = await primary.evaluate(element => {
          const body = getComputedStyle(element); const heading = getComputedStyle(element.closest('[data-frame-unit]').querySelector('.frameProblem'));
          return {family:body.fontFamily,size:parseFloat(body.fontSize),headingSize:parseFloat(heading.fontSize),headingWeight:Number(heading.fontWeight)};
        });
        ...
        await noOverflow(result.current);
        chapter.checked.push(unit.unit_id); result.units++;
      }
      if (frame.units.length) {
        await page.locator(`[data-frame-unit-link="${frame.units[0].unit_id}"]`).click(); await settle();
        ...
        if (!sampled.has(row.subject)) {
          ...
          const {nodeId} = await cdp.send('DOM.querySelector',{nodeId:doc.root.nodeId,selector:'[data-frame-unit]:not([hidden]) .framePrimary'});
          const observation = await cdp.send('CSS.getPlatformFontsForNode',{nodeId});
          result.fontObservations.push({subject:row.subject,...observation});
    ...
    for (const {row,frame,href} of representatives) {
      for (const width of [1728,1024,390]) {
        ...
        await page.goto(href); await page.locator('[data-politics-frame]').waitFor(); await settle();
        for (const unit of frame.units) {
        ...
        await page.screenshot({path:path.join(directory,`${row.subject}-${width}.png`)});
        result.responsive.push({subject:row.subject,width,units:frame.units.length});
      }
    }
    // Two equally useful maps must remain simultaneous, not tabs or accordions.
    await page.setViewportSize({width:1728,height:1117});
    await page.goto(`${base}/politics/marxism/ch00/`); await page.locator('[data-politics-frame]').waitFor();
    const links = page.locator('[data-frame-unit-link]');
    if (await links.count()>1) {
      await links.nth(1).click(); await settle();
      const panels = page.locator('[data-frame-unit]:visible .role-framework_maps > .frameValues.parallel > .frameValue');
      ...
      await page.evaluate(()=>scrollTo(0,0));
      await page.screenshot({path:path.join(directory,'marxism-c00-s02-two-maps.png'),fullPage:true});
    }
    ...
    result.status='PASS'; delete result.current;
    ...
    result.status='FAIL'; result.failure=error.stack;
