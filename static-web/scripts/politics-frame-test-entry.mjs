...
export async function runCurrentPoliticsFrame({subject,formal}){
 ...
 console.log('Current acceptance: native frame -> Workbench'+(formal?' / Home contract + full formal 23-group regression':` / ${subject}`));
 ...
 // Existing upload-artifact paths must contain the actual observations, not just a pointer.
 if(formal){
  ...
  const files=['src/components/PoliticsHomeTools.astro','scripts/test-politics-home-journey.mjs','scripts/test-politics-frame-journey.mjs'];
