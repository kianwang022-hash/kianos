...
const blockedFidelityFixtureId = 'POL27-CF-MARX-C00-K04-N02-I01';
const practiceQuestion = practiceCatalog.questions.find((q) =>
  q.chengfengLocator?.status === 'EXACT_SOURCE_NODE'
  && q.chengfengLocator?.sourceNodeId
  && q.chengfengLocator.sourceNodeId !== blockedFidelityFixtureId
);
if (!practiceQuestion) throw new Error('POLITICS_PRACTICE_VISUAL_SAFE_LOCATOR_FIXTURE_MISSING');
...
try {
  ...
  check(
    practiceQuestion.chengfengLocator?.sourceNodeId !== blockedFidelityFixtureId,
    'practice_result_exact_locator_excludes_blocked_fidelity_node',
    String(practiceQuestion.chengfengLocator?.sourceNodeId || '')
  );
