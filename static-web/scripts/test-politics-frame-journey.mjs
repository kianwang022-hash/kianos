...
import { testPoliticsSubjectReadability } from './test-politics-subject-readability.mjs';
...
try{
 ...
 for(const subject of ['marxism','mao','history','xi','ethics_law']){
  ...
  const type=await p.evaluate(()=>({stem:parseFloat(getComputedStyle(document.querySelector('.politicsStem')).fontSize),options:[...document.querySelectorAll('.politicsOptions button')].map(el=>parseFloat(getComputedStyle(el).fontSize)),exitWidth:document.querySelector('[data-exit-session]').getBoundingClientRect().width}));
  assert.ok(type.stem>=22&&type.options.length&&type.options.every(size=>size>=19)&&type.exitWidth<200,JSON.stringify(type));report.typography.push({subject,...type});
  await p.screenshot({path:path.join(out,subject+'-question-1440.png')});
  await p.setViewportSize({width:390,height:900});assert.equal(await p.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+1),true,`${subject} narrow question overflow`);await p.screenshot({path:path.join(out,subject+'-question-390.png')});
  report.checks.push(subject+' native Current geometry / Chengfeng anchor / exact NU scope / clean persistence / 22px stem + 19px options + compact toolbar / narrow fallback');console.log('PASS',report.checks.at(-1));await context.close();
 }
