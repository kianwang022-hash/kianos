...
assert(systemWuReturn.includes("['wrong','uncertain'].includes(String(result.status || ''))"), 'component-wu-filter-missing');
assert(systemWuReturn.includes('const relation = question?.relation;'), 'component-does-not-derive-canonical-relation');
assert(systemWuReturn.includes('!relation?.blockId || !relation?.primaryKpId || !route'), 'component-safety-contract-missing');
...
assert(bridge.includes('XIZONG_MEMORY_STORAGE_KEY'), 'bridge-does-not-merge-into-current-memory-repair-owner');
assert(bridge.includes('const next = setRepairTasks(memory, [...preserved, ...incoming]);'), 'bridge-import-repair-task-missing');
assert(!bridge.includes('appendMemoryEvidence') && !bridge.includes('evidence.push'), 'bridge-repair-role-missing');
assert(bridge.includes("window.addEventListener('storage'"), 'open-block-tab-cannot-receive-repair-inbox');
const bridgeMemoryWriteIndex = bridge.indexOf('if (!writeJson(XIZONG_MEMORY_STORAGE_KEY, next))');
const bridgeInboxClearIndex = bridge.indexOf('localStorage.removeItem(inboxKey)', bridgeMemoryWriteIndex);
assert(bridgeMemoryWriteIndex >= 0 && bridgeInboxClearIndex > bridgeMemoryWriteIndex, 'repair-inbox-consumption-does-not-persist-before-clear');
assert(systemWuReturn.includes('XIZONG_MEMORY_STORAGE_KEY'), 'visible-memory-repair-delivery-missing');
assert(systemWuReturn.includes('nextMemory=setRepairTasks(memory,[...kept,...tasks]);'), 'visible-memory-repair-owner-bypassed');
