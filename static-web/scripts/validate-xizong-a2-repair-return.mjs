...
assert(systemWuReturn.includes("!['wrong','uncertain'].includes(String(result.status || ''))"), 'system-wu-filter-missing');
assert(systemWuReturn.includes('const relation = question?.relation;'), 'system-return-does-not-derive-canonical-relation');
assert(systemWuReturn.includes('!relation?.blockId || !relation?.primaryKpId || !route'), 'system-return-safety-contract-missing');
...
assert(component.includes('Return 无效、作答已变化，或未能安全保存；没有应用本次计划。'), 'invalid-return-not-contained');
...
assert(systemWuReturn.includes('setRepairTasks'), 'visible-memory-repair-owner-bypassed');
