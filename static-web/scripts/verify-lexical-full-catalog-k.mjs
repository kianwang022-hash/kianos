...
const truthyReviewFlag = (value) => value === true || value === 1 || value === '1' || String(value).toLowerCase() === 'true';
...
fail(relationFiles.length === Number(relationManifest.relation_count), 'RELATION_OWNER_COUNT_DRIFT', `${relationFiles.length}|${relationManifest.relation_count}`);
...
for (const ordinal of ordinals) {
  ...
  fail(!truthyReviewFlag(record.needs_delta_review), 'OWNER_NEEDS_DELTA_REVIEW', ordinal);
  ...
  fail(senses.length >= 1, 'NO_ACTIVE_SENSE', `${ordinal}:${word}`);
  const activeIds = new Set();
  const identitySenseRows = Array.isArray(raw.identity_refs?.senses) ? raw.identity_refs.senses : [];
  const identityById = new Map(identitySenseRows.map((row) => [String(row?.sense_id || ''), row]));
  ...
  for (const [senseIndex, sense] of senses.entries()) {
    ...
    const identityRow = identityById.get(sid);
    fail(identityRow?.status === 'active', 'ACTIVE_SENSE_LIFECYCLE_NOT_ACTIVE', `${ordinal}:${sid}:${identityRow?.status || 'missing'}`);
    fail(nonempty(sense.definition_cn) || nonempty(sense.definition_en), 'ACTIVE_SENSE_DEFINITION_EMPTY', `${ordinal}:${sid}`);
    fail(!truthyReviewFlag(sense.needs_human_review), 'ACTIVE_SENSE_NEEDS_HUMAN_REVIEW', `${ordinal}:${sid}`);
  ...
  for (const ref of references) {
    ...
    fail(!activeIds.has(sid), 'REFERENCE_SENSE_STILL_ACTIVE', `${ordinal}:${sid}`);
    fail(ref?.reference_only === true, 'REFERENCE_SENSE_NOT_MARKED_REFERENCE_ONLY', `${ordinal}:${sid}`);
    if (sid) {
      const identityRow = identityById.get(sid);
      fail(Boolean(identityRow), 'REFERENCE_SENSE_MISSING_IDENTITY_ROW', `${ordinal}:${sid}`);
      fail(identityRow?.status !== 'active', 'REFERENCE_SENSE_IDENTITY_STILL_ACTIVE', `${ordinal}:${sid}`);
    }
  }
  ...
  fail(lookupSpellings.has(word), 'PRIMARY_SPELLING_LOOKUP_MISSING', `${ordinal}:${word}`);

  const formIdentity = record.form_identity && typeof record.form_identity === 'object' ? record.form_identity : null;
  if (formIdentity) {
    structuralCounts.form_identity += 1;
    for (const boundary of (Array.isArray(formIdentity.boundaries) ? formIdentity.boundaries : [])) {
      if (nonempty(boundary?.surface)) fail(lookupSpellings.has(String(boundary.surface)), 'FORM_BOUNDARY_LOOKUP_MISSING', `${ordinal}:${boundary.surface}`);
    }
    for (const variant of (Array.isArray(formIdentity.variants) ? formIdentity.variants : [])) {
      const surface = String(variant?.canonical_form || variant?.surface || variant?.spelling || '');
      if (surface) fail(lookupSpellings.has(surface), 'FORM_VARIANT_LOOKUP_MISSING', `${ordinal}:${surface}`);
    }
  }
  ...
  for (const [index, construction] of constructions.entries()) {
    const id = String(construction?.construction_id || construction?.fact_id || '');
    fail(nonempty(id), 'CONSTRUCTION_STABLE_ID_MISSING', `${ordinal}:${index}`);
    fail(nonempty(construction?.pattern) || nonempty(construction?.boundary) || nonempty(construction?.label_en), 'CONSTRUCTION_PATTERN_EMPTY', `${ordinal}:${index}`);
  }
  ...
  const status = riskFlags.length ? 'DEPTH_READY' : 'SAFE_SIMPLE';
  ...
  if (!representatives.rich_polysemous && senses.length >= 3) representatives.rich_polysemous = { ordinal, word_id: raw.word_id, word };
  if (!representatives.familiar_new && hasHiddenBranchCandidate && senses.length >= 2) representatives.familiar_new = { ordinal, word_id: raw.word_id, word };
  if (!representatives.construction_heavy && constructions.length >= 2) representatives.construction_heavy = { ordinal, word_id: raw.word_id, word };
  if (!representatives.relation_owned && rawRefs.length >= 1) representatives.relation_owned = { ordinal, word_id: raw.word_id, word };
  if (!representatives.register_sensitivity && hasRegisterSignal) representatives.register_sensitivity = { ordinal, word_id: raw.word_id, word };
  if (!representatives.same_owner_form && formIdentity) representatives.same_owner_form = { ordinal, word_id: raw.word_id, word };
  if (!representatives.reference_only && references.length >= 1) representatives.reference_only = { ordinal, word_id: raw.word_id, word };
}
...
for (const key of ['safe_simple','rich_polysemous','familiar_new','construction_heavy','relation_owned','register_sensitivity','same_owner_form','reference_only','distinct_owner_spelling']) {
  fail(Boolean(representatives[key]), 'REPRESENTATIVE_RISK_FAMILY_MISSING', key);
}

fail(conclusions.length === 7946, 'CONCLUSION_COUNT_NOT_7946', conclusions.length);
fail(qualityCounts.SAFE_SIMPLE + qualityCounts.DEPTH_READY === 7946 && qualityCounts.BLOCKED === 0, 'QUALITY_TERMINAL_ACCOUNTING_DRIFT', JSON.stringify(qualityCounts));
...
const head = (() => {
  try { return String(process.env.GITHUB_SHA || '').trim() || 'local'; } catch { return 'local'; }
})();
...
const report = {
  ...
  source_truth: {
    ...
    relation_owner_count: relationFiles.length
  },
...
fs.writeFileSync(path.join(acceptanceRoot, 'full-catalog-k-conclusions.json'), JSON.stringify({
  schema: 'kianos.lexical.full_catalog_k_conclusions.v1',
  issue: 56,
  head,
  owner_count: conclusions.length,
  digest_sha256: conclusionDigest,
  conclusions
}, null, 2) + '\n');
fs.writeFileSync(path.join(acceptanceRoot, 'full-catalog-k-report.json'), JSON.stringify(report, null, 2) + '\n');
console.log(JSON.stringify(report, null, 2));

assert.equal(failures.length, 0, `FULL_CATALOG_K_PHASE_AB_FAILURES:${JSON.stringify(failures.slice(0, 40))}`);
