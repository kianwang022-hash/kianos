  ...
  try {
    ...
    const metrics = await page.evaluate(() => {
      ...
      return { url: location.pathname, viewport: { width: innerWidth, height: innerHeight }, scroll: { width: document.documentElement.scrollWidth, height: document.documentElement.scrollHeight }, bodyFont: getComputedStyle(document.body).fontFamily, bodySize: getComputedStyle(document.body).fontSize,
        visible: ['.productBar','.productCanvas','.homeGrid','.homeLane','.homeCurrentStrip','.englishResume','.englishCapabilityLayout','.englishCapabilityGroup','.englishFirstLearning','.portedReadingWorkspace','.portedReadingPassage','.portedReadingQuestions','.readingWorkspace','.readingPassage','.readingQuestionPane','h1'].flatMap(rect),
        localStorageKeys: Object.keys(localStorage).sort() };
    });
    const cdp = await context.newCDPSession(page);
    await cdp.send('DOM.enable'); await cdp.send('CSS.enable');
    const { root: doc } = await cdp.send('DOM.getDocument');
    const { nodeId } = await cdp.send('DOM.querySelector', { nodeId: doc.nodeId, selector: 'body' });
    const platformFonts = await cdp.send('CSS.getPlatformFontsForNode', { nodeId });
    await page.screenshot({ path: path.join(out, `${name}.png`), fullPage: false });
    if (metrics.scroll.height > viewport.height + 3) await page.screenshot({ path: path.join(out, `${name}-full.png`), fullPage: true });
