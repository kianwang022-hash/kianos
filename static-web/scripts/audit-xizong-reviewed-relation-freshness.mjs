...
const resolveFreshness = createXizongReviewedRelationFreshnessResolver({ repoRoot });
...
for (const row of rows) {
  ...
  const freshness = resolveFreshness(row);
  const status = freshness.status;
  const ownerPath = freshness.knowledge_path;
  ...
  const bucket = byOwner.get(ownerPath) || {
    owner_path: ownerPath,
    current_blob_sha: freshness.current_blob_sha,
    current: 0,
    stale: 0,
    owner_missing: 0,
    witness_missing: 0,
  ...
  if (status !== 'CURRENT') reportRows.push({
    ...
    current_blob_sha: freshness.current_blob_sha,
    knowledge_blob_sha: freshness.knowledge_blob_sha,
    knowledge_revalidated_blob_sha: freshness.knowledge_revalidated_blob_sha,
    effective_review_witness: freshness.effective_review_witness
  });
}
