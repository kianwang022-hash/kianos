...
const isBlockSpecificSelector = (selector) => xizongScope.test(selector) && blockSelector.test(selector);

function splitMixedBlockSelector(selector) {
  if (!selector.includes('.xv6Block') || xizongScope.test(selector)) return null;
  const matches = [...selector.matchAll(/:is\(([^()]*)\)/g)];
  const target = matches.find((match) => match[1].split(',').map((item) => item.trim()).includes('.xv6Block'));
  if (!target) return null;
  const alternatives = target[1].split(',').map((item) => item.trim()).filter(Boolean);
  const remaining = alternatives.filter((item) => item !== '.xv6Block');
  if (!remaining.length) return null;
  const full = target[0];
  const kept = selector.slice(0, target.index) + `:is(${remaining.join(',')})` + selector.slice(target.index + full.length);
  const moved = selector.slice(0, target.index) + '.xzBlockWorkspace' + selector.slice(target.index + full.length);
  return { kept, moved };
}
...
function partitionCss(source) {
  ...
  while (cursor < source.length) {
    ...
    for (const selector of selectors) {
      if (isBlockSpecificSelector(selector)) {
        movedSelectors.push(selector);
        continue;
      }
      const mixed = splitMixedBlockSelector(selector);
      if (mixed) {
        keptSelectors.push(mixed.kept);
        movedSelectors.push(mixed.moved);
      } else {
...
for (const file of broadFiles) {
  ...
  if (/\.xv6Block\b/.test(source) && file.endsWith('site-visual-tuning.css')) throw new Error('MIXED_XV6_BLOCK_SELECTOR_STILL_PRESENT_IN_SITE_TUNING');
}
for (const file of componentFiles) {
...
for (const token of ['.portedStudyLayout', '.portedStudyChain', '.portedOutlineSectionList', '.xv6LogicStage', '.xv6BlockWorkspaceShell', '.xv6LearnerAuxSurface', '.xv6KpLearnCompanion']) {
...
const dense = read('static-web/src/styles/xizong-dense-calm.css');
for (const token of ['.portedStudyExtension', '.portedPrecisionShell', '.portedReviewWorkspace']) {
  if (!dense.includes(token)) throw new Error(`AFTER_LEARN_RULE_LOST:${token}`);
}
...
console.log(JSON.stringify({
  ...
  mixed_selector_split: 'xv6Block removed from cross-domain :is rules',
  aux_layout_handoff: 'data-only',
  important_recovery: false,
  after_learn_rules_preserved: true
}, null, 2));
