...
has(repairUi, '当前没有审核过的精确 Block/KP 回链：保留题号，不自动猜。', 'no-guessed-repair-route');

has(systemWuReturn, 'assertCurrentWuBinding(row, currentWu.get(row.question_id) || null)', 'chat-plan-not-scoped-to-real-wu');
has(systemWuReturn, '!relation?.blockId || !relation?.primaryKpId || !route', 'repair-route-not-reviewed-relation-only');
...
has(repairBridge, 'XIZONG_MEMORY_STORAGE_KEY', 'repair-inbox-does-not-use-current-memory-owner');
has(repairBridge, 'const next = setRepairTasks(memory, [...preserved, ...incoming]);', 'repair-inbox-does-not-create-repair-task');
...
has(repairBridge, 'kianos:xizong-repair-inbox-migrated', 'repair-return-runtime-notify-missing');
