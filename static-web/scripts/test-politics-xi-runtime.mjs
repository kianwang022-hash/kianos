...
const auditDir = path.resolve(process.cwd(), '../politics-xi-runtime-audit');
fs.mkdirSync(auditDir, { recursive: true });
const report = { schema: 'kianos.politics.xi_runtime_journey.v1', started_at: new Date().toISOString(), checks: [] };
const check = (ok, name, detail = '') => {
  if (!ok) throw new Error(`POLITICS_XI_RUNTIME_FAIL:${name}${detail ? `:${detail}` : ''}`);
...
async function waitForServer() {
  ...
  throw new Error('POLITICS_XI_PREVIEW_SERVER_NOT_READY');
}
...
async function findConfig(page, minQuestions = 1) {
  ...
  check(Boolean(fallback), `xi_config_with_${minQuestions}_questions_exists`);
  return fallback;
}
...
async function smoke(page) {
  ...
  for (let i = 0; i <= 17; i += 1) {
    ...
    check((await root.getAttribute('data-chapter')) === chapter, `xi_${chapter}_identity`);
    check(await root.locator('[data-politics-unit]').count() > 0, `xi_${chapter}_units`);
    check(await root.locator('[data-politics-external-source]').count() > 0, `xi_${chapter}_external_chengfeng`);
    check(await page.locator('[data-politics-cognitive-workspace]').count() === 0, `xi_${chapter}_generic_runtime`);
  }
}
async function cleanJourney(page) {
  await clearState(page);
  const located = await findConfig(page, 1); check(Boolean(located), 'xi_clean_config_found');
  const { config } = located; const ids = config.expected_question_ids || [];
  for (let i = 0; i < ids.length; i += 1) {
    const card = await answer(page, ids[i], 'correct');
    check((await attempt(page, config, ids[i]))?.outcome === 'STABLE', `xi_clean_attempt_${i + 1}`);
  ...
  check((await unitReturn.textContent())?.includes('不等于长期掌握'), 'xi_clean_no_mastery');
  const events = await page.evaluate(() => JSON.parse(localStorage.getItem('kianos-politics-evidence-v1') || '[]'));
  check(Array.isArray(events) && events.length === 0, 'xi_clean_no_repair_debt');
}
async function resumeJourney(page) {
  await clearState(page);
  const located = await findConfig(page, 2); check(Boolean(located), 'xi_resume_config_found');
  ...
  check(before?.subject === 'xi' && before?.chapter === chapter, 'xi_resume_identity', JSON.stringify(before));
  check(before?.action === 'VERIFY' && before?.question_id === second, 'xi_resume_pending_question', JSON.stringify(before));
  await page.reload({ waitUntil: 'domcontentloaded' });
  const restored = cardFor(page, second); await restored.waitFor({ state: 'visible' });
  check(await restored.evaluate((node) => node.classList.contains('active')), 'xi_refresh_restores_pending');
}
async function repairJourney(page) {
  await clearState(page);
  const located = await findConfig(page, 1); check(Boolean(located), 'xi_repair_config_found');
  ...
  check(['WRONG','UNCERTAIN'].includes(first?.outcome), 'xi_problem_first_attempt', JSON.stringify(first));
  const repair = card.locator('[data-politics-repair]'); await repair.waitFor({ state: 'visible' });
  const sourceLink = repair.locator('a[href^="#source-"]'); await sourceLink.click();
  ...
  check(atSource?.action === 'REPAIR_SOURCE' && atSource?.subject === 'xi' && atSource?.chapter === chapter, 'xi_repair_source_cursor', JSON.stringify(atSource));
  await returnLink.click(); await card.waitFor({ state: 'visible' });
  check(await card.locator('[data-politics-result]').isVisible(), 'xi_repair_original_result_visible');
  check(await card.locator('[data-politics-repair]').isVisible(), 'xi_repair_panel_visible');
  check(JSON.stringify(await attempt(page, config, qid)) === JSON.stringify(first), 'xi_repair_first_attempt_immutable');
}
...
try {
  ...
  console.log('POLITICS_XI_RUNTIME_PASS'); console.log(JSON.stringify({ checks: report.checks.length }));
} catch (error) {
