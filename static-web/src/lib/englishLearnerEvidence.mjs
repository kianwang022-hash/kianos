...
export function restoreEnglishCheckpoint(storage,payload){
 ...
 for(const [key,raw] of Object.entries(incoming)){
  if(key.startsWith(archivePrefix)&&local[key]!=null&&local[key]!==raw)throw new Error('ENGLISH_CHECKPOINT_CONFLICT_KEEP_LOCAL:'+key);
 }
 const combined={...incoming,...local};
 const retired=inspectEnglishCheckpoint({schema:payload.schema,entries:combined}).retired_current;
 const changes=[];
 for(const [key,raw] of Object.entries(incoming)){
  if(retiredCurrent(key,raw,combined,retired))continue;
  const existing=storage.getItem(key);
  if(existing!=null&&existing!==raw)throw new Error('ENGLISH_CHECKPOINT_CONFLICT_KEEP_LOCAL:'+key);
