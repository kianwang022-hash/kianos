...
function xizongForecastBlockRecords(record){
  ...
  const rows=fs.readdirSync(absolute(blocksPath))
    ...
    .map((name)=>{
      ...
      const field=(key)=>{
        const match=frontmatter.match(new RegExp(`^${key}\\s*:\\s*(.+)import crypto from 'node:crypto';
import fs from 'node:fs';
import path from 'node:path';
...
,'m'));
  ...
  if(expectedBlocks&&rows.length!==expectedBlocks){
    throw new Error(
      `CURRENT_XIZONG_FORECAST_BLOCK_COUNT_MISMATCH:${record.identity.systemId}:${rows.length}/${expectedBlocks}`
    );
  }
  const kpSum=rows.reduce((sum,row)=>sum+row.kpCount,0);
  if(expectedKp&&kpSum!==expectedKp){
    throw new Error(
      `CURRENT_XIZONG_FORECAST_KP_COUNT_MISMATCH:${record.identity.systemId}:${kpSum}/${expectedKp}`
    );
  }
