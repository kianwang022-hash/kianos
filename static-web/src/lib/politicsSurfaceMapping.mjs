...
function selectGroupIndices(value, group) {
  const owner = oneObject(value, `POLITICS_SURFACE_GROUPS:${group.id}`);
  const rows = Array.isArray(owner.groups) ? owner.groups : [];
  return group.select_group_indices.map((index, position) => {
    const item = rows[index];
    if (!present(item)) throw new Error(`POLITICS_SURFACE_GROUP_INDEX_MISSING:${group.id}:${index}`);
    const id = Array.isArray(group.item_ids) && group.item_ids[position]
      ? group.item_ids[position]
