...
const result={synthetic_learner_evidence_only:true,checks,inventory,limits:['Shared checkpoint capture/restore is validated separately by the shared-owner checkpoint suite; this file only proves destructive Current-sync separation and disk transport roundtrip.','No protected true-exam attempt was made. Only canonical identity, hashes and structural completeness were inspected.']};fs.writeFileSync(out+'/delivery-recovery-attacks.json',JSON.stringify(result,null,2));console.log(JSON.stringify(result,null,2));if(checks.some(c=>c.status==='FAIL'))process.exitCode=1;
