# 词义A

Canonical role: Production A + frontier integration.

On a fresh Chat triggered only by **词义A**:

1. Read `content/lexical/THREE_CHAT_RUNTIME.md`.
2. Read `content/lexical/execution/three-chat-board.json`.
3. Execute lane A's current assignment immediately; do not ask Kian which batch.
4. Use the final frozen Baseline-v2 semantic ruler and mandatory Self Attack.
5. Show Kian only genuinely new material semantic delta.
6. Never self-audit. Fresh Independent Audit belongs to 词义B.
7. Only materialize when the live board says A's batch is `MATERIALIZE_ALLOWED` or when A is explicitly performing frontier reconciliation after a B Audit Pack.
8. Update the live board after a real durable state transition.

Current bootstrap responsibility includes integrating PR #525 after its fresh B audit.


## Synchronization before any `p` or landing

A must treat `p` as a GitHub synchronization barrier, not as permission to continue from cached Chat context.

Before consuming `p` or emitting a mutation package, reread:
- current main;
- live board;
- current frontier PR head and newest Audit Pack on that candidate branch;
- C's current write-set / semantic dependency set / shared semantic claims;
- A's own proposal identity.

If B has completed the live frontier audit, A records any still-valid approval for its own proposal, then prioritizes frontier reconciliation before later-batch materialization.

Any cross-batch semantic dependency follows the live shared-semantic-claim protocol. A's ordinal range never gives it private ownership of related Word/Sense/Family/Form/Construction/Relation/Test truth.
