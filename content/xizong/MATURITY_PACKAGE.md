...
The shared maturity standard is a highest-bar question set. This package is Xizong's self-contained subject-native Current adaptation; it must not be replaced by the generic S/K/L/P/R/E engineering gate order.
...
## Owner boundary — anti-drift

This package is routed directly by `content/xizong/CURRENT.md`.

It integrates bounded outputs from:

- `CONTENT_MAINLINE.md` / exact content owners → material and Source truth only;
- `static-web/src/lib/xizongForecastModel.mjs` → workload / uncertainty / sensitivity evidence only;
- exact S/K/L/P/R/E owners → execution-readiness lookup only when a selected learner action is blocked;
- private learner/runtime evidence → Real Learner U.

Hard rule:

> **No supporting lane may choose the Xizong maturity stage, the 275 bottleneck, or the next learner action by itself.**

`CONTENT_MAINLINE.md` is not the parent of subject maturity. Forecast is not a strategy brain. S/K/L/P/R/E is not the stage order.
...
- Working target: **275+**
- Elastic upside: **>275**
- Working loss envelope: **25 / 300**
- Protected-floor loss envelope: **30 / 300**
- Working-target reserve above protect floor: **5 points**

The 5-point reserve is a planning buffer between 275 and 270, not a probability guarantee and not permission to spend five points casually.

Current historical exam-format owner:

`content/xizong/questions/exam-format.json`

Current authoritative geometry in that owner ends at **2026**:

- 2017–2026 historical era: **165 questions / 300 points**
- 2027 exact exam geometry is **not yet owned by this file**

Therefore:

> 165 / 300 is the current historical baseline for planning and stress tests, not an authoritative 2027-format claim.

Current Stage-A gap:

- `CURRENT_YEAR_DELTA_GAP: 2027 exam-format / scoring geometry`

Until Current 2027 authority exists, the 25-point loss envelope may use historical geometry as a prior but must retain this uncertainty.

---

# 2｜Capability formation matrix

| Capability | Formation material | Normal method | Stronger evidence | Current system readiness | Real learner status |
| --- | --- | --- | --- | --- | --- |
| X1 Source / Understanding / System Model | original Lecture/MarginNote + canonical System/Block/KP/LG | Source contact + mechanism reconstruction | coherent explanation/reconstruction against Current owner | medical owner topology exists across A1–F | **UNKNOWN until real U** |
| X2 Active Recall | KP Core + Recall surfaces | Core-hidden active retrieval | repeated/delayed successful retrieval | runtime/learning semantics exist | **UNKNOWN until real U** |
...
> System/runtime readiness never promotes a learner capability to Secure.

---

# 3｜Current exam-scope / material matrix

Current canonical medical identity:

- **8 first-class System owners**
- **159 numbered Blocks**
- **2517 numbered canonical KPs**
- retained O9 tumor-general overlay remains separate from the 2517 total

| Scope | Canonical medical owner | Medical content status | Exact official System-question scope | Current Stage-A interpretation |
| --- | --- | --- | --- | --- |
| A1 Circulation | present · 12 Blocks / 312 KP | existing Current owner | **CURRENT · 376 qids** | no known hard medical-content gap from Stage-A evidence |
...
# 4｜Known material / source gaps

## G1 — Non-System clinical-medical humanistic coverage
...
- bind 2027 official format when authoritative source exists;
- until then keep historical 165/300 as planning prior only;
- reopen loss-budget geometry if 2027 changes materially.
...
Protected work is not deleted merely to hit a date.

## Score formation

    X5 Precision
    + X6 decisive discrimination
    + X7 causal Repair / fresh verify / delayed stability
    + X8 case / cross-System transfer

Stable areas become cheaper; targeted work replaces curriculum replay.

## Late review / simulation
...
| Claim | Minimum legitimate evidence family |
| --- | --- |
| Source/model formed | source contact + coherent model evidence |
| Recallable | active retrieval evidence |
| Block/LG stable | local reconstruction / delayed evidence |
| System stable | System reconstruction + representative application |
| Precision stable | selective exact retrieval + delay |
| Official application stable | representative official attempts, exposure-aware |
| Repair closed | changed-context / delayed verification, not same-item correction |
| Case/cross-System capable | dedicated case/cross-System evidence |
| Whole-paper execution | sealed full-paper evidence under known contamination conditions |
| 275 defensible | score evidence + coverage/loss reconciliation + no material hidden capability gap |
...
The historical discipline profile and prior whole-paper data may inform stress tests, but they do not close the 2027 loss envelope.

---

# 8｜Stage-A priority queue
...
This package coordinates; exact owners still own truth.

- Content / Material Closure:
  - current executable hard-coverage baseline is closed;
  - future Source deltas or concrete material defects are repaired at exact owners;
  - returns Gap Matrix evidence here without owning maturity.
- Current Forecast kernel:
  - prices accepted workload/material inputs;
  - consumes real learner evidence when available;
  - exposes checkpoint work buckets, uncertainty and sensitivity;
  - does not declare Stage A–D mature or choose the next System.
- Question→Knowledge Crosswalk:
  - continues progressive reviewed mapping;
  - missing mapping stays legal and is not guessed.
- Website/runtime:
  - remains execution transport;
  - no reopening without concrete learner-visible/durability defect.
...
## Forecast system-logic status — CLOSED

Current canonical supporting implementation:

- `static-web/src/lib/xizongForecastModel.mjs`
- `static-web/scripts/test-xizong-forecast-model.mjs`
- falsifiability surface: `buildXizongForecastFalsifiability(...)`

Current boundaries proven on Current main:

- 5h/day capacity is an explicit scenario, not an assumed schedule;
- 30% W/U stress is supported without overwriting observed System-specific rates;
- Case/Cram and other late-stage assets enter only as `gross - replacement - overlap = net new`;
- 10/20 checkpoint output decomposes remaining evidence-priced work into Learn / Attempt / Reconstruct / Repair / Verify / Formal-score-calibrate buckets;
- observed score bands remain descriptive when contamination is unresolved;
- stronger score extrapolation requires a separate calibration band with enough protected / fresh-equivalent evidence;
- Forecast returns `subject_stage_decision = OUT_OF_SCOPE`.

Therefore Forecast system logic is usable as a supporting lane, while Kian-specific precision remains Real-U dependent.
...
Current Forecast kernel is a supporting evidence lane.
...
Current acceptance also proves stale / invented identity rejection, idempotent identical replay, conflict fail-closed, browser-reset recovery and failed-read no-replacement safety. Shared Home additionally has a **capacity-conflict veto**: if known Chat Plan target minutes exceed current usable daily capacity, automatic Next Action is withheld and the decision returns to Chat; Website does not reschedule.
...
# 37｜Mother-standard closure verdict

The mother standard requires one subject-native system that can connect target score, trainable capability, material, method, evidence, Forecast, dynamic control, future Source and low-friction execution without turning Website or engineering completion into a strategy brain. That requirement is now satisfied at **system-logic level**.

## Closure facts

- Score path is explicit: 270 protect, 275 working, >275 elastic upside.
- A1–F exact System-question membership is closed; clinical humanities has its own baseline owner.
- D/E/F medical Content is closed and is not reopened for symmetry.
- X1–X9 each have a named formation and evidence path.
- Current material inventory distinguishes Source / official / Repair / Transfer / Stress / synthetic / prior-year / future / reference-only roles.
- Source fidelity, semantic revision and downstream invalidation are fail-closed.
- Synthetic transfer probes now require target / decision axis / failure mechanism / transfer rule / distractor mechanism and remain TRANSFER_ONLY.
- Forecast now exposes workload bands, stress scenarios, checkpoint work buckets, sensitivity grid, decision-flip surfaces and next-information evidence; it cannot advance subject Stage.
- Home now vetoes a known Chat Plan whose target minutes exceed usable daily capacity and returns the decision to Chat rather than rescheduling.
- Typed Chat Return, exact interrupted Resume, stale/revision rejection, idempotent replay and private durable recovery are already accepted integration capabilities.
- Future-source baseline / delta / overlap / replacement / fallback semantics exist for the known late-stage families.
- False Secure and False Unstable have explicit counter-rules.
- Fresh Chat and No-Website reasoning remain possible because semantic/control truth is durable outside the UI.
- Authentic physical-modality cost remains an explicit later calibration variable rather than silently equated to browser timing.

## Remaining Unknowns

Only these remain legitimate:

1. authoritative unreleased / unrecovered 2027 Source deltas;
2. bounded upstream Source bytes already named by Current owners;
3. real Kian learner evidence;
4. authentic-modality calibration where it can change X9 / formal-score interpretation;
5. low-value unknowns that do not change the current learner action.

None of these justify more architecture now.

## Current learner-control verdict

The current binding uncertainty is **Real Learner U**, not repository completeness.

Therefore the valid next loop is:

    real Stage-B study
    → Source/model + Recall + reconstruction + official application evidence
    → Chat diagnoses the actual binding capability
    → smallest Repair / Verify when needed
    → Forecast recalibrates
    → stable work becomes cheaper

Not:

    inspect next untested engineering gate
    → build it for symmetry
    → repeat
