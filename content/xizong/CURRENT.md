...
**Next action:** execute `static-web/XIZONG_UI_MIGRATION.md` **Slice 4 — bounded legacy cleanup + gated shared Shell adoption** from current `main`. PR #368 closed System Framework, PR #370 closed Home, PR #371 closed standalone Memory, and PR #374 closed System Exit / official Question with exact Runtime semantics preserved. First audit legacy selector reachability for those already-isolated surfaces and delete only selectors no Current DOM can reach. Preserve Block presentation rules until Block receives an explicit ownership decision. Shared Base Shell / collapsible global `K` rail adoption may happen only after the English-owned implementation is accepted on `main`; if it has not landed, do not reproduce or patch it locally.
