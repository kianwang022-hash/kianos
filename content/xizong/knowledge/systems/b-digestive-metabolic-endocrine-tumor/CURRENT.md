...
**Active / earliest unresolved eligible gate:** `K — bounded M5–M7 lipid Source→Knowledge hierarchy rebase`.  
**S status:** PASS — exact Current B official-question membership accepted at 1071 qids.  
**K status:** **BOUNDED_REOPEN — M/G only**; D1–D23 remain accepted. **M1–M4 are Current after canonical hierarchy reconstruction + direct 27 Source mapping**; M5–M10/G1–G5 remain inside the bounded reopen.  
**L status:** previous PASS remains historical evidence; **M1–M4 Learning now consume canonical hierarchies directly through the global Biochemistry Source map**; remaining affected M/G Learning stays frozen behind K.  
...
**Next action:** rebase **M5–M7 lipid metabolism** using the same bottom-up chain: current 27 Source → canonical Knowledge hierarchy → direct Source mapping → Learning consumption. Do not touch website/UI.
...
- Initial audit found all 15 M/G Blocks on the old 26 Source basis. **M1–M4 are now rebuilt on the 27 Source with explicit canonical hierarchies and direct Source→Knowledge→Learning mapping**; M5–M10/G1–G5 remain under bounded rebase.
