...
Role: the single root routing entry for engineering/status requests. Normal learning or normal use of a KianOS product starts from the relevant domain/product owner or private Runtime path, not this engineering index. This file stores no learner/user state, subject cursor or duplicate acceptance ledger.
...
Chat interprets, diagnoses and plans. GitHub preserves canonical Rule / Content / Visual / Engineering assets. The website is a product execution surface; learning surfaces are one major subtype. Domain-native Runtime owns evidence semantics; private Runtime preserves learner/execution facts and transports instructions/receipts where applicable. Codex executes a bounded approved task; background preparation stays quiet when it has no useful work.
...
After a local repair, verify the affected interface and stop. Reopen for a real defect, new Source, platform change or explicit user need. Material visual/product decisions remain Kian's; live hosting and real private learner/execution-state mutation are separately authorized.
